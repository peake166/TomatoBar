...
TomatoBar 是一款简洁高效的番茄工作法计时器，常驻在您的 macOS 菜单栏。它旨在帮助您通过番茄工作法（一种时间管理方法）来提高工作和学习效率，保持专注。此增强版在原版基础上进行了大量功能拓展和优化，使其更加灵活和用户友好。
...
3.  **时间块视图:**
    ...
    *   鼠标悬停在时间块上会显示暂停/继续、编辑和删除按钮。点击删除按钮会弹出确认框。
    *   右键点击时间块可进行编辑、删除等操作。
    *   拖拽时间块可以调整顺序。
...
*   **`TomatoBar/State.swift`:**
    ...
    *   `TimeBlockManager`: 核心状态管理器，负责：
        ...
        *   提供 `refreshAllTimeBlocks` 方法重置所有时间块。
...
*   **应用状态:** 当前活动时间块的索引、剩余秒数、已完成工作块数等状态信息保存在 `~/Library/Containers/com.github.ivoronin.TomatoBar/Data/Documents/timeBlockState.json` 文件中。应用退出或状态改变时保存。
...
*   **更高的自定义度：** 支持自定义各类时间块（工作、短休息、长休息）的时长、颜色和名称
*   **时间块提醒功能：** 可为每个时间块设置一个或多个自定义提醒，支持独立声音开关
*   **菜单栏快捷操作：** 优化了菜单栏右键菜单，支持直接切换时间块和控制计时器
*   **开机自启动：** 增加了应用开机自动启动的选项，让您的时间管理更加便捷
*   **状态同步与恢复：** 改进了状态保存和恢复机制，可在应用重启后恢复之前的计时状态
*   **原生中文支持：** 默认使用中文界面，为中文用户提供更友好的体验
